...
namespace detail {
constexpr float featureDefault(contract::FeatureIndex f) noexcept {
  return contract::kFeatureDefaults[contract::index(f)];
}
} // namespace detail
struct MeasuredValue {
  float value = 0.0F;
  bool valid = false;
  std::uint64_t age_ms = kUnknownMeasurementAgeMs;
};
struct ClimateMeasurements {
  MeasuredValue air_temperature_c{detail::featureDefault(contract::FeatureIndex::AirTemperatureC),
                                  false, kUnknownMeasurementAgeMs};
  MeasuredValue relative_humidity_pct{
      detail::featureDefault(contract::FeatureIndex::RelativeHumidityPct), false,
      kUnknownMeasurementAgeMs};
  MeasuredValue co2_ppm{detail::featureDefault(contract::FeatureIndex::Co2Ppm), false,
                        kUnknownMeasurementAgeMs};
  MeasuredValue outside_temperature_c{
      detail::featureDefault(contract::FeatureIndex::OutsideTemperatureC), false,
      kUnknownMeasurementAgeMs};
  MeasuredValue outside_humidity_pct{
      detail::featureDefault(contract::FeatureIndex::OutsideHumidityPct), false,
      kUnknownMeasurementAgeMs};
};
struct TrendValue {
...
struct ClimateTrends {
  TrendValue temperature{};
  TrendValue humidity{};
  TrendValue co2{};
};
struct ClimateState {
...
struct ClimateTargets {
  ...
  float relative_humidity_pct =
      detail::featureDefault(contract::FeatureIndex::TargetRelativeHumidityPct);
...
struct ClimateSchedule {
  float light_level = detail::featureDefault(contract::FeatureIndex::LightLevel);
};
...
struct ClimateControllerInput {
  ClimateState state{};
  HumidityControlMode humidity_control_mode = HumidityControlMode::Rh;
...
struct ClimateFeatureVector {
  std::array<float, contract::kFeatureCount> values{};
};
struct ClimateEncoderReport {
  std::uint64_t substituted_feature_mask = 0U, clamped_feature_mask = 0U;
  bool substituted(contract::FeatureIndex f) const noexcept {
    return (substituted_feature_mask & (std::uint64_t{1U} << contract::index(f))) != 0U;
  }
  bool clamped(contract::FeatureIndex f) const noexcept {
    return (clamped_feature_mask & (std::uint64_t{1U} << contract::index(f))) != 0U;
  }
};
