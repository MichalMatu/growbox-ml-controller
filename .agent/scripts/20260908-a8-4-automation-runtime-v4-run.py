...
if out(['git', 'rev-parse', 'HEAD']) != BASE:
    raise SystemExit('A8_4_V4_IDENTITY_FAIL local HEAD mismatch')
if out(['git', 'rev-parse', 'FETCH_HEAD']) != BASE:
    raise SystemExit('A8_4_V4_IDENTITY_FAIL remote branch mismatch')
if out(['git', 'status', '--porcelain']):
    raise SystemExit('A8_4_V4_IDENTITY_FAIL worktree not clean')
print('A8_4_V4_IDENTITY_PASS')
...
old_target = r'''    "climate/output/OutputSupervisorLifecycle.cpp"\n    "climate/output/OutputExecutionProjection.cpp"'''
fixed_target = r'''    "climate/output/OutputSupervisorLifecycle.cpp"\n    "climate/output/OutputLifecycleExecutor.cpp"\n    "climate/output/OutputExecutionProjection.cpp"'''
old_replacement = r'''    "climate/output/OutputSupervisorLifecycle.cpp"\n    "climate/output/OutputAutomationControl.cpp"\n    "climate/output/OutputExecutionProjection.cpp"'''
fixed_replacement = r'''    "climate/output/OutputSupervisorLifecycle.cpp"\n    "climate/output/OutputLifecycleExecutor.cpp"\n    "climate/output/OutputAutomationControl.cpp"\n    "climate/output/OutputExecutionProjection.cpp"'''
if generator.count(old_target) != 1:
    raise SystemExit(f'A8_4_V4_GENERATOR_FIX_FAIL old_target_count={generator.count(old_target)}')
if generator.count(old_replacement) != 1:
    raise SystemExit(f'A8_4_V4_GENERATOR_FIX_FAIL old_replacement_count={generator.count(old_replacement)}')
generator = generator.replace(old_target, fixed_target, 1)
generator = generator.replace(old_replacement, fixed_replacement, 1)
Path('/tmp/a8_4_edit_v4.py').write_text(generator)
Path('/tmp/a8_4_fix.py').write_text(
    out(['git', 'show', 'FETCH_HEAD:.agent/scripts/20260908-a8-4-automation-runtime-v1-fix.py']) + '\n')
print('A8_4_V4_GENERATOR_FIX_PASS')
run(['python3', '/tmp/a8_4_edit_v4.py'])
...
if changed != EXPECTED:
    raise SystemExit(f'A8_4_V4_ALLOWLIST_FAIL changed={changed!r}')
...
if 'supervisor_context.mode = automation_report.mode;' not in runtime:
    raise SystemExit('A8_4_V4_STATIC_FAIL runtime does not use lifecycle mode')
if 'supervisor_context.mode = output::SupervisorMode::Automatic;' in runtime:
    raise SystemExit('A8_4_V4_STATIC_FAIL hard-coded automatic mode remains')
if 'automation_control.tick(now_ms, schedule_intent, safety_snapshot.envelope)' not in runtime:
    raise SystemExit('A8_4_V4_STATIC_FAIL automation tick missing')
if 'hardSafetyActive' not in control or 'SafetyDeferred' not in control:
    raise SystemExit('A8_4_V4_STATIC_FAIL safety defer missing')
if 'OutputLifecycleCommand::DisableAutomation' not in control or 'OutputLifecycleCommand::EnableAutomation' not in control:
    raise SystemExit('A8_4_V4_STATIC_FAIL lifecycle ON/OFF commands missing')
if 'equalsIgnoreCase(tokens[0],"automation")' not in console_cmd:
    raise SystemExit('A8_4_V4_STATIC_FAIL service command missing')
print('A8_4_V4_STATIC_PASS')

build = 'build/host-tests-a8-4-v4'
...
print('A8_4_V4_FOCUSED_PASS')

fw_build = 'build/stage27c-a8-4-v4'
...
print('A8_4_V4_CANONICAL_BUILD_PASS')
...
if staged != EXPECTED:
    raise SystemExit(f'A8_4_V4_STAGED_ALLOWLIST_FAIL staged={staged!r}')
...
if out(['git', 'rev-parse', 'FETCH_HEAD']) != commit:
    raise SystemExit('A8_4_V4_PUSH_VERIFY_FAIL')
if out(['git', 'status', '--porcelain']):
    raise SystemExit('A8_4_V4_CLEAN_FAIL')
print(f'A8_4_V4_PASS commit={commit}')
