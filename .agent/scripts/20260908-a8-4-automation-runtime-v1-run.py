...
if out(['git', 'rev-parse', 'HEAD']) != BASE:
    raise SystemExit('A8_4_IDENTITY_FAIL local HEAD mismatch')
if out(['git', 'rev-parse', 'FETCH_HEAD']) != BASE:
    raise SystemExit('A8_4_IDENTITY_FAIL remote branch mismatch')
if out(['git', 'status', '--porcelain']):
    raise SystemExit('A8_4_IDENTITY_FAIL worktree not clean')
print('A8_4_IDENTITY_PASS')

run(['git', 'fetch', '-q', 'origin', 'agent-control'])
Path('/tmp/a8_4_edit.py').write_text(out(['git', 'show', 'FETCH_HEAD:.agent/scripts/20260908-a8-4-automation-runtime-v1.py']) + '\n')
Path('/tmp/a8_4_fix.py').write_text(out(['git', 'show', 'FETCH_HEAD:.agent/scripts/20260908-a8-4-automation-runtime-v1-fix.py']) + '\n')
run(['python3', '/tmp/a8_4_edit.py'])
...
if changed != EXPECTED:
    raise SystemExit(f'A8_4_ALLOWLIST_FAIL changed={changed!r}')
...
if 'supervisor_context.mode = automation_report.mode;' not in runtime:
    raise SystemExit('A8_4_STATIC_FAIL runtime does not use lifecycle mode')
if 'supervisor_context.mode = output::SupervisorMode::Automatic;' in runtime:
    raise SystemExit('A8_4_STATIC_FAIL hard-coded automatic mode remains')
if 'automation_control.tick(now_ms, schedule_intent, safety_snapshot.envelope)' not in runtime:
    raise SystemExit('A8_4_STATIC_FAIL automation tick missing')
if 'hardSafetyActive' not in control or 'SafetyDeferred' not in control:
    raise SystemExit('A8_4_STATIC_FAIL safety defer missing')
if 'OutputLifecycleCommand::DisableAutomation' not in control or 'OutputLifecycleCommand::EnableAutomation' not in control:
    raise SystemExit('A8_4_STATIC_FAIL lifecycle ON/OFF commands missing')
if 'equalsIgnoreCase(tokens[0],"automation")' not in console_cmd:
    raise SystemExit('A8_4_STATIC_FAIL service command missing')
print('A8_4_STATIC_PASS')

build = 'build/host-tests-a8-4-v1'
...
print('A8_4_FOCUSED_PASS')

fw_build = 'build/stage27c-a8-4-v1'
...
print('A8_4_CANONICAL_BUILD_PASS')
...
if staged != EXPECTED:
    raise SystemExit(f'A8_4_STAGED_ALLOWLIST_FAIL staged={staged!r}')
...
if out(['git', 'rev-parse', 'FETCH_HEAD']) != commit:
    raise SystemExit('A8_4_PUSH_VERIFY_FAIL')
if out(['git', 'status', '--porcelain']):
    raise SystemExit('A8_4_CLEAN_FAIL')
print(f'A8_4_PASS commit={commit}')
