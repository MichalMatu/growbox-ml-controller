...
print('A3_1_STATIC_PASS')
PY

cmake -S test/host -B build/host-tests-a3-1-v1
cmake --build build/host-tests-a3-1-v1 --parallel --target \
  output_state_store_tests output_execution_contract_tests
ctest --test-dir build/host-tests-a3-1-v1 \
  -R '^(output_state_store_tests|output_execution_contract_tests)$' --output-on-failure
echo A3_1_FOCUSED_PASS
