...
if source.count(old_guard) != 1:
    raise SystemExit('A10_1_V3_PATCH_FAIL static guard mismatch')
source = source.replace(old_guard, new_guard, 1)

marker = "run(['git', 'add', '-N',"
if source.count(marker) != 1:
    raise SystemExit('A10_1_V3_PATCH_FAIL runner marker mismatch')
fixup = r'''
...
for old, new in replacements:
    ...
    if count != 1:
        raise SystemExit(f'A10_1_V3_FIXUP_FAIL expected one malformed CRLF, found {count}')
    console_bytes = console_bytes.replace(old, new, 1)
console_path.write_bytes(console_bytes)
print('A10_1_V3_STRING_FIXUP_PASS')

'''
source = source.replace(marker, fixup + marker, 1)
Path('/tmp/a10_1_v3_inner.py').write_text(source)
subprocess.run(['python3', '/tmp/a10_1_v3_inner.py'], check=True)
