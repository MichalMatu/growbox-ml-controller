...
def build_metrics(build_dir):
    ...
    return {
        ...
        'data': data_size,
        'bss': bss_size,
        'static_data_bss': data_size + bss_size,
...
# Formatting/lint/schema stage. Any auto-fix is a failure for the immutable final SHA.
...
# Single repository-wide pre-push software quality stage. Firmware builds are skipped here
# because the exact fake/real CrowPanel profiles are executed once below.
...
# Required exact firmware profiles, build-only: no serial, flash, monitor, or RF hardware access.
