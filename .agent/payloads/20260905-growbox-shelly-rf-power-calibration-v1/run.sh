...
EXPECTED_BRANCH_HEAD=c249a598fd069e709e0eaaacff129cf790177110
...
def set_master(on):
    rpc("Switch.Set", {"id": 0, "on": bool(on), "tag": "growbox-cal"})
    time.sleep(0.6)
...
def sample_power(count=7, interval=0.35):
    rows = []
...
output_path = Path("/tmp/growbox_shelly_rf_power_calibration_v1.json")
output_path.write_text(json.dumps(results, indent=2, sort_keys=True))
print(output_path.read_text())
print(
    "SHELLY_RF_POWER_CALIBRATION_COMPLETE "
