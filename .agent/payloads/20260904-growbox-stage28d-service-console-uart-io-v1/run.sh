...
TASK=20260904-growbox-stage28d-service-console-uart-io-v1
...
s = p.read_text()
old = '''The real-input runtime now includes a bounded primary-serial service console on the same ESP-IDF primary stdio/UART interface used by the normal CrowPanel CH340 monitor. Output remains on primary stdio; command input uses the ESP-IDF driver for the configured primary console UART so CH340 RX is consumed non-blockingly.'''
new = '''The real-input runtime now includes a bounded primary-serial service console on the same configured UART used by the normal CrowPanel CH340 monitor. Service-console RX and TX use the ESP-IDF UART driver directly, while normal ESP-IDF logging keeps its existing primary logger transport. This avoids dependence on newlib stdin/stdout routing for command traffic.'''
if old not in s:
    raise SystemExit('CURRENT_STATUS UART transport paragraph missing')
s = s.replace(old, new, 1)
p.write_text(s)

p = Path('continuation.md')
s = p.read_text()
old = '''It keeps output on ESP-IDF primary stdout and consumes command input directly from the ESP-IDF driver for the configured primary console UART, so the qualified CrowPanel uses the same CH340 serial monitor for logs and commands without a separate USB Serial/JTAG channel.'''
new = '''It uses the ESP-IDF UART driver directly for service-console RX and TX on the configured primary console UART, while normal ESP-IDF logs keep their existing logger transport. The qualified CrowPanel therefore uses the same CH340 serial monitor for logs and commands without a separate USB Serial/JTAG channel or newlib stdin/stdout dependency for service traffic.'''
if old not in s:
    raise SystemExit('continuation UART transport sentence missing')
s = s.replace(old, new, 1)
p.write_text(s)
PY
...
test -x "$PC"
set +e
"$PC" run --files src/climate/runtime/Stage28ServiceConsole.cpp docs/CURRENT_STATUS.md continuation.md
FIRST_RC=$?
set -e
...
cmake -S test/host -B build/host-stage28d-service-console-uart-io -DCMAKE_BUILD_TYPE=Debug >/tmp/stage28d-console-uart-io-host-cmake.log
cmake --build build/host-stage28d-service-console-uart-io --target stage28_service_console_tests rf433_protocol_tests --parallel 1
./build/host-stage28d-service-console-uart-io/stage28_service_console_tests
./build/host-stage28d-service-console-uart-io/rf433_protocol_tests
...
STAGE27C_BUILD_DIR=build/idf-stage28d-service-console-uart-io-crowpanel \
  bash scripts/stage27c_crowpanel.sh build

SDKCONFIG=build/idf-stage28d-service-console-uart-io-crowpanel/sdkconfig
test -f "$SDKCONFIG"
...
git add src/climate/runtime/Stage28ServiceConsole.cpp docs/CURRENT_STATUS.md continuation.md
git commit -m "Use UART driver for Stage28D service console I/O"
...
TIDY_COPY=/tmp/run_clang_tidy_host_stage28d_console_uart_io_v1.sh
