...
with serial.Serial(port, 115200, timeout=0.12, write_timeout=1.0) as handle:
    ...
    if not status_ready(handle):
        raise SystemExit("firmware/safety identity check failed before fan actuation")
    ...
    try:
        ...
        transmit_with_evidence(
            handle,
            "rf fan on",
            "manual_rf_tx device=fan state=on code=906118656",
        )
        print("FAN_ON_LOCAL_PASS")
        time.sleep(4.0)
        transmit_with_evidence(
            handle,
            "rf fan off",
            "manual_rf_tx device=fan state=off code=1040336384",
        )
        print("FAN_OFF_LOCAL_PASS")
    except Exception:
        if on_attempted:
            try:
                for _ in range(3):
                    text = command(handle, "rf fan off", 1.5)
                    if "manual_rf_tx device=fan state=off code=1040336384" in text:
                        break
            except Exception:
                pass
        raise
    ...
    if not status_ready(handle):
        raise SystemExit("safe runtime/RF status check failed after fan actuation")

text = b"".join(all_chunks).decode("utf-8", errors="replace")
Path("/tmp/stage28d_manual_rf_fan_v1.log").write_text(text)
print(text[-12000:])
...
print(
    "STAGE28D_MANUAL_RF_FAN_V1_LOCAL_PASS "
