#!/usr/bin/env bash
set -euo pipefail
EXPECTED=edd4a2328a7d0d43bccd3becab67500b9948a2e6
...
cat > docs/CONTINUATION_PLAN.md <<'EOF'
...
if s.count(old) != 1:
    raise SystemExit('unexpected README status match count')
...
test -x "$PC"
"$PC" run --files continuation.md docs/CONTINUATION_PLAN.md README.md || {
  git diff --check
  "$PC" run --files continuation.md docs/CONTINUATION_PLAN.md README.md
}
git diff --check
...
python3 - <<'PY'
import subprocess
allowed = {'continuation.md', 'docs/CONTINUATION_PLAN.md', 'README.md'}
...
if set(paths) != allowed:
    raise SystemExit('unexpected changed path set: ' + ', '.join(paths))
PY

git add continuation.md docs/CONTINUATION_PLAN.md README.md
git commit -m "Prepare fresh-chat project handoff"
...
printf 'FRESH_CHAT_HANDOFF_READY commit=%s parent=%s hardware_tested=%s docs_only=1 stage28d_started=0 precommit=pass\n' "$NEW" "$EXPECTED" 316b58e76de609069ddbf2667fe86f6218fb2143
