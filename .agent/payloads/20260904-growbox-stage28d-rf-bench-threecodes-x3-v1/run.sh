...
ORIG=/tmp/stage28d-rf-bench-threecodes-x3-v1.orig.cpp
TEST_BUILD=build/idf-stage28d-rf-bench-threecodes-x3-v1
SAFE_BUILD=build/idf-stage28d-rf-bench-threecodes-x3-v1-safe
OUT=/tmp/stage28d-rf-bench-threecodes-x3-v1
...
echo "STAGE28D_RF_BENCH_PORT=$PORT"
echo "STAGE28D_RF_BENCH_SCOPE rf_only=1 mains_loads_disconnected=1 cycles_per_pair=3"
...
echo "STAGE28D_RF_BENCH_FLASH_TEST lamp_x3 fan_x3 humidifier_x3 pulse_us=560 repeat=10"
scripts/stage27c_crowpanel.sh flash

rm -rf "$OUT"
.venv/bin/python tools/stage27c_soak.py --port "$PORT" --output-dir "$OUT" --duration 55 --progress-seconds 5 --expected-sha "$EXPECTED"
...
for path in sorted(out.glob('raw-*.log')):
    for line in path.read_text(encoding='utf-8', errors='replace').splitlines():
        if 'rf433_loopback_v=1 ' in line:
            lines.append(line)
sequence = [
    ('lamp','ON',235030016),('lamp','OFF',16926208),('lamp','ON',235030016),('lamp','OFF',16926208),('lamp','ON',235030016),('lamp','OFF',16926208),
    ('fan','ON',906118656),('fan','OFF',1040336384),('fan','ON',906118656),('fan','OFF',1040336384),('fan','ON',906118656),('fan','OFF',1040336384),
    ('humidifier','ON',637683200),('humidifier','OFF',771900928),('humidifier','ON',637683200),('humidifier','OFF',771900928),('humidifier','ON',637683200),('humidifier','OFF',771900928),
]
assert len(lines) == 18, f'expected 18 TX lines, got {len(lines)}'
for index, (line, (device, state, code)) in enumerate(zip(lines, sequence), 1):
    fields = {}
    for token in line.split('rf433_loopback_v=1 ', 1)[1].split():
        if '=' in token:
            k, v = token.split('=', 1)
            fields[k] = v
    assert int(fields['requested_code'], 0) == code
    assert int(fields['requested_bits'], 0) == 32
    assert int(fields['requested_protocol'], 0) == 2
    assert int(fields['requested_repeat'], 0) == 10
    assert int(fields['requested_pulse_us'], 0) == 560
    assert int(fields['tx_queued'], 0) == 1
    assert int(fields['tx_started'], 0) == 1
    assert int(fields['tx_completed'], 0) == 1
    assert fields.get('outputs') == 'fake-locked'
    print(f'STAGE28D_RF_BENCH_FRAME index={index} device={device} state={state} code={code} local_rx_pass={fields.get("pass","unknown")} tx_completed=1')
print('STAGE28D_RF_BENCH_TX_SEQUENCE_PASS')
PY
...
echo "STAGE28D_RF_BENCH_RESTORE_SAFE_FIRMWARE"
scripts/stage27c_crowpanel.sh flash

test -z "$(git status --porcelain)"
printf 'STAGE28D_RF_BENCH_DONE sha=%s frames=18 cycles_per_pair=3 pulse_us=560 repeat=10 safe_firmware_restored=1\n' "$EXPECTED"
