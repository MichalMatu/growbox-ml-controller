...
std::size_t formatStage27TelemetryNdjson(char* buffer, std::size_t buffer_size,
  ...
  const int written = std::snprintf(
      ...
      ",\"scan_restarts\":%" PRIu32 ",\"scan_completes\":%" PRIu32
      ",\"adv_lock_drops\":%" PRIu32 "},"
      ...
      ",\"heap_psram_min\":%" PRIu32 ",\"heap_psram_largest\":%" PRIu32
      ",\"stack_free\":%" PRIu32 "},"
      "\"storage\":{\"mounted\":%s,\"mount_errors\":%" PRIu32 ",\"write_errors\":%" PRIu32
      ",\"queue_drops\":%" PRIu32 ",\"records_written\":%" PRIu32
      ",\"records_skipped\":%" PRIu32 ",\"last_write_ms\":%" PRIu64 "}}",
      snapshot.uptime_ms, snapshot.unix_time_s, snapshot.reset_reason, jsonBool(snapshot.input_sampled),
      snapshot.io_status, jsonBool(snapshot.scd_sample), jsonBool(snapshot.scd_available),
      static_cast<double>(snapshot.scd_temperature_c), static_cast<double>(snapshot.scd_humidity_pct),
      static_cast<double>(snapshot.scd_co2_ppm), snapshot.scd_age_ms, snapshot.scd_read_errors,
      ...
      snapshot.tp_age_ms, snapshot.tp_packets, snapshot.tp_accepted, snapshot.tp_rejected,
      jsonBool(snapshot.xiaomi_sample), static_cast<double>(snapshot.xiaomi_temperature_c),
      ...
      static_cast<double>(snapshot.applied_exhaust_fan), static_cast<double>(snapshot.applied_humidifier),
      static_cast<double>(snapshot.applied_dehumidifier), static_cast<double>(snapshot.applied_co2_doser),
      snapshot.heap_internal, snapshot.heap_internal_min, snapshot.heap_internal_largest,
      snapshot.heap_psram, snapshot.heap_psram_min, snapshot.heap_psram_largest, snapshot.stack_free,
