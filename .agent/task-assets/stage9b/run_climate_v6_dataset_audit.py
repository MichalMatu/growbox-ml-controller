...
def _label_coverage(labels: np.ndarray, splits: np.ndarray) -> dict[str, dict[str, dict[str, float | int]]]:
    ...
    for split in ("train", "validation", "test"):
        ...
        for index, name in enumerate(CLIMATE_OUTPUT_NAMES):
            ...
            per_output[name] = {
                ...
                "active_p50": float(np.quantile(active_values, 0.50)) if active_values.size else 0.0,
                "active_p90": float(np.quantile(active_values, 0.90)) if active_values.size else 0.0,
...
def _vpd_output_coverage(labels: np.ndarray, modes: np.ndarray) -> dict[str, dict[str, float | int]]:
    mask = modes == "VPD"
...
def main() -> None:
    ...
    expected_rows = len(REQUIRED_SCENARIO_FAMILIES) * config.scenarios_per_family * config.steps_per_scenario
    print(
    ...
    for split in ("train", "validation", "test"):
        strong = {name: label_coverage[split][name]["strong_count"] for name in CLIMATE_OUTPUT_NAMES}
