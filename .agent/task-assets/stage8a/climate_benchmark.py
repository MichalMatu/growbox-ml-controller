...
from .climate_input import ClimateInputConfig, ClimateTrendEstimator, MeasurementStatus, air_vpd_kpa, encode_climate_input
...
def _status_for_step(episode: ClimateTrainingEpisode, step: int, total: int) -> dict[str, MeasurementStatus]:
...
def _tracking_terms(state, profile, status: dict[str, MeasurementStatus], timeout_ms: int) -> tuple[float, bool, float, float | None, float | None, float | None]:
    ...
    if profile.humidity_control_mode == "RH":
        ...
        vpd_error = abs(air_vpd_kpa(state.air_temperature_c, state.relative_humidity_pct) - targets.air_vpd_kpa)
    ...
    if profile.humidity_control_mode == "RH":
        ...
        humidity_weight = 4.0
    tracking_cost = 6.0 * temp_excess * temp_excess + humidity_weight * humidity_excess * humidity_excess + 1.5 * co2_excess * co2_excess
...
def _aggregate(policy: PolicyName, episodes: list[EpisodeMetrics]) -> AggregateMetrics:
    ...
    vpd_mae = (
        sum(item.vpd_mae_kpa * item.vpd_steps for item in episodes) / vpd_steps if vpd_steps else 0.0
    )
    co2_mae = (
        sum(item.co2_mae_ppm * item.co2_steps for item in episodes) / co2_steps if co2_steps else 0.0
    )
    return AggregateMetrics(
        ...
        co2_dose_command_seconds_per_episode=sum(item.co2_dose_command_seconds for item in episodes) / len(episodes),
