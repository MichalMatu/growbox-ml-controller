...
def test_freshness_is_independent_from_validity() -> None:
    ...
    record = climate_controller_record(ClimateScenario(), ClimateState(co2_ppm=1_200.0), status=status)
...
def test_vpd_falls_back_when_required_measurement_is_stale() -> None:
    ...
    default_vpd = next(feature.default for feature in contract.features if feature.name == "air_vpd_kpa")
...
def test_active_targets_and_mode_are_encoded_without_wall_clock() -> None:
    ...
    record = climate_controller_record(ClimateScenario(), ClimateState(light_level=0.8), config=config)
