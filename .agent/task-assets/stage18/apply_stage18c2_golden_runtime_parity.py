...
CPP_TEST = dedent(r'''
    ...
    std::array<float, 6>
    values(const growbox::climate::EstimatedEffectiveClimateActions& action) {
    ...
    public:
      ...
      bool infer(const growbox::climate::ClimateFeatureVector&,
        ...
        if (fixture_.model_behavior ==
            growbox::climate::parity_fixture::ModelBehavior::Fail) {
          return false;
        }
    ...
    makeInput(const growbox::climate::parity_fixture::Case& fixture) {
      ...
      auto measured = [&](std::size_t index) {
        return MeasuredValue{fixture.measurement_values[index],
                             fixture.measurement_valid[index],
      ...
      input.capabilities = {
          fixture.capabilities[0], fixture.capabilities[1], fixture.capabilities[2],
    ...
    void runCase(const growbox::climate::parity_fixture::Case& fixture) {
      ...
      check(decision.ml_evaluated == fixture.expected_ml_evaluated, fixture.name,
            "ML evaluated");

      checkArray(values(decision.rule.raw), fixture.expected_rule_raw, fixture.name, "rule raw");
      checkArray(values(decision.rule.arbitrated), fixture.expected_rule_arbitrated,
                 fixture.name, "rule arbitrated");
      checkArray(values(decision.rule.safe), fixture.expected_rule_safe, fixture.name,
                 "rule safe");
      check(decision.rule.arbitration_interventions == fixture.expected_rule_arb_mask,
            fixture.name, "rule arbitration interventions");
      check(decision.rule.safety_interventions == fixture.expected_rule_safety_mask,
            fixture.name, "rule safety interventions");

      if (fixture.expected_ml_evaluated) {
        checkArray(values(decision.ml.raw), fixture.expected_ml_raw, fixture.name, "ML raw");
        checkArray(values(decision.ml.arbitrated), fixture.expected_ml_arbitrated,
                   fixture.name, "ML arbitrated");
        checkArray(values(decision.ml.safe), fixture.expected_ml_safe, fixture.name, "ML safe");
        check(decision.ml.arbitration_interventions == fixture.expected_ml_arb_mask,
              fixture.name, "ML arbitration interventions");
        check(decision.ml.safety_interventions == fixture.expected_ml_safety_mask,
              fixture.name, "ML safety interventions");
      }
      ...
      checkArray(values(decision.applied), fixture.expected_applied, fixture.name,
                 "applied action");
      checkArray(values(decision.effective_before), fixture.expected_effective_before,
                 fixture.name, "effective before");
      checkArray(values(decision.effective_after), fixture.expected_effective_after,
                 fixture.name, "effective after", 2.0e-4F);
    }
    } // namespace
...
write("tools/ml/generate_climate_runtime_parity.py", GENERATOR)
write("tests/test_climate_runtime_parity.py", PYTEST)
write("test/test_climate_runtime_parity/test_main.cpp", CPP_TEST)

cmake_path = ROOT / "test/host/CMakeLists.txt"
cmake = cmake_path.read_text(encoding="utf-8")
if "climate_runtime_parity_tests" not in cmake:
    marker = 'target_compile_options(climate_v6_tests PRIVATE -Wall -Wextra -Wpedantic)\n'
    block = dedent(r'''

        add_executable(
          climate_runtime_parity_tests
          "${PROJECT_ROOT}/test/test_climate_runtime_parity/test_main.cpp"
        ...
        target_include_directories(
          climate_runtime_parity_tests
          PRIVATE
            "${PROJECT_ROOT}/lib/environment_control/src/climate"
            "${PROJECT_ROOT}/tests/fixtures"
        )
        target_compile_features(climate_runtime_parity_tests PRIVATE cxx_std_17)
        target_compile_options(climate_runtime_parity_tests PRIVATE -Wall -Wextra -Wpedantic)
    ''')
    if marker not in cmake:
        raise SystemExit("climate_v6 compile-options marker not found")
    cmake = cmake.replace(marker, marker + block, 1)
    cmake = cmake.replace(
    ...
    cmake = cmake.replace(
        ...
        "add_test(NAME climate_v6_tests COMMAND climate_v6_tests)\n"
        "add_test(NAME climate_runtime_parity_tests COMMAND climate_runtime_parity_tests)\n",
        1,
    )
