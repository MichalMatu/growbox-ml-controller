...
class ClimateDatasetBundle:
    ...
    def __post_init__(self) -> None:
        ...
        if any(len(values) != rows for values in (self.families, self.profiles, self.safe_fallbacks)):
...
def audit_climate_dataset(
    ...
    *,
    ...
    missing_families = [family for family in REQUIRED_SCENARIO_FAMILIES if family not in family_counts]
    if missing_families:
