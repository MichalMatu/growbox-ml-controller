...
def build_training_episode(
    ...
    if family == "cold_heating":
        ...
        state = replace(state, co2_ppm=float(rng.uniform(410.0, 550.0)), light_level=1.0)
        caps = replace(caps, co2_doser=Co2DoserCapabilities(available=False, max_injection_ppm_s=0.0))
        first = _profile(
            "co2-unavailable",
            temperature=state.air_temperature_c,
            humidity=state.relative_humidity_pct,
        ...
        if missing == "heater":
            ...
            first = _profile("missing-humidifier", temperature=state.air_temperature_c, humidity=65.0)
            ...
            first = _profile("missing-dehumidifier", temperature=state.air_temperature_c, humidity=58.0)
        else:
