...
class ClimateSimulator:
    ...
    def _advance_effective_action(self, command: ClimateAction, dt: float) -> None:
        ...
        self.effective_action = ClimateAction(
            ...
            exhaust_fan=self._lag(
                old.exhaust_fan, command.exhaust_fan, dt, lag.exhaust_fan_s
            ),
            humidifier=self._lag(old.humidifier, command.humidifier, dt, lag.humidifier_s),
            dehumidifier=self._lag(
                old.dehumidifier, command.dehumidifier, dt, lag.dehumidifier_s
            ),
    ...
    def _advance_physics(self, dt: float) -> None:
        ...
        heater_w = (
            effective.heater * max(0.0, caps.heater.max_power_w) * _clamp(caps.heater.efficiency, 0.0, 1.0)
        )
        ...
        vapour_g_m3 = (
            _clamp(vh_rh, 0.0, 100.0) / 100.0 * sat_absolute_humidity_g_m3(vh_t)
        )
        humidifier_g_s = (
        ...
        injection_ppm = (
            effective.co2_doser * max(0.0, caps.co2_doser.max_injection_ppm_s) * dt
        )
