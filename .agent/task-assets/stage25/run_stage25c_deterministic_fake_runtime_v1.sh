...
EXPECTED="${2:-}"
ASSET='.agent/task-assets/stage25/apply_stage25c_deterministic_fake_runtime_v1.py'
...
case "$MODE" in
  apply)
    ...
    git show "origin/agent-control:$ASSET" > /tmp/stage25c-apply.py
    .venv/bin/python /tmp/stage25c-apply.py

    .venv/bin/pre-commit run clang-format --files \
      src/climate/ClimateDeterministicFake.h \
      src/climate/ClimateDeterministicFake.cpp \
      src/climate/ClimateV6FakeRuntime.cpp \
      test/test_climate_deterministic_fake_runtime/test_main.cpp || true
    .venv/bin/pre-commit run clang-format --files \
      src/climate/ClimateDeterministicFake.h \
      src/climate/ClimateDeterministicFake.cpp \
      src/climate/ClimateV6FakeRuntime.cpp \
      test/test_climate_deterministic_fake_runtime/test_main.cpp
    ...
    ! grep -E -i 'gpio_set|gpio_config|i2c_|scd4|nimble|ble_|ds3231|pcf8563|shelly|mqtt|modbus' \
      src/climate/ClimateDeterministicFake.h src/climate/ClimateDeterministicFake.cpp
    grep -F 'deterministic-240-tick-v1' src/climate/ClimateV6FakeRuntime.cpp
    git status --short
    ;;

  focused)
    cmake -S test/host -B build/host-tests
    cmake --build build/host-tests --target climate_deterministic_fake_runtime_tests --parallel
    ctest --test-dir build/host-tests -R '^climate_deterministic_fake_runtime_tests$' --output-on-failure
    ...
    grep -F 'Stage25C replaces the fixed smoke snapshot' docs/CURRENT_STATUS.md
    grep -F '### Stage25C completed — deterministic fake runtime' docs/CONTINUATION_PLAN.md
    git diff --check
    ;;
  ...
  full)
    ...
    rm -rf build/idf-gate-stage25c-legacy build/idf-gate-stage25c-fake
    IDF_GATE_BUILD_DIR=build/idf-gate-stage25c-legacy \
      IDF_GATE_APP_MODE=legacy bash scripts/idf_gate_build.sh
    IDF_GATE_BUILD_DIR=build/idf-gate-stage25c-fake \
    ...
    git commit -m 'Add deterministic climate v6 fake scenario runtime'
