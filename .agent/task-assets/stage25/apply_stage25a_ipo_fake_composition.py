...
write(
    ...
    bool same(const ClimatePolicyRequest& left, const ClimatePolicyRequest& right) {
      return near(left.heater, right.heater) && near(left.cooler, right.cooler) &&
             ...
             near(left.dehumidifier, right.dehumidifier) &&
             near(left.co2_doser, right.co2_doser);
    }
    ...
    public:
      ...
      bool apply(ClimateActuatorRole role, float level,
        ...
        const bool batch_ok =
            batch >= batch_outcomes_.size() ? true : batch_outcomes_[batch];
    ...
    void testMlShadowDivergesButRuleSafeOutputRemainsAuthoritative() {
      ...
      ScriptedSnapshotProvider provider(
          {{true, snapshotFor(20.0F)}, {true, snapshotFor(21.0F)}});
      RecordingRoleDriver driver{};
      ClimateApplication application(runtime, provider, driver);
    ...
    void testRejectedCommandGetsOffRecoveryWithoutPoisoningConfirmedState() {
      ScriptedSnapshotProvider provider(
          {{true, snapshotFor(20.0F)}, {true, snapshotFor(20.0F)}});
...
src_marker = '    "climate/ClimateIoAdapters.cpp"\n'
assert src_marker in src_text
assert '"climate/ClimateApplication.cpp"' not in src_text
src_cmake.write_text(
    src_text.replace(src_marker, src_marker + '    "climate/ClimateApplication.cpp"\n', 1),
    encoding="utf-8",
)

host_cmake = ROOT / "test" / "host" / "CMakeLists.txt"
host_text = host_cmake.read_text(encoding="utf-8")
assert "climate_application_composition_tests" not in host_text
host_insert = dedent(
    r'''

    add_executable(
      climate_application_composition_tests
      "${PROJECT_ROOT}/test/test_climate_application_composition/test_main.cpp"
    ...
    target_compile_features(climate_application_composition_tests PRIVATE cxx_std_17)
    target_compile_options(climate_application_composition_tests PRIVATE -Wall -Wextra -Wpedantic)
    ''')
host_marker = "\nif(UNIX)\n"
assert host_marker in host_text
host_text = host_text.replace(host_marker, host_insert + host_marker, 1)
...
host_text = host_text.replace(
    link_marker,
    link_marker + "  target_link_libraries(climate_application_composition_tests PRIVATE m)\n",
    1,
)
test_marker = "add_test(NAME climate_io_adapter_tests COMMAND climate_io_adapter_tests)"
assert test_marker in host_text
host_text = host_text.replace(
    test_marker,
    test_marker
    + "\nadd_test(NAME climate_application_composition_tests COMMAND climate_application_composition_tests)",
    1,
)
host_cmake.write_text(host_text, encoding="utf-8")

tidy = ROOT / "scripts" / "run_clang_tidy_host.sh"
tidy_text = tidy.read_text(encoding="utf-8")
tidy_marker = "  lib/environment_control/src/climate/ClimateTrendEstimator.cpp\n"
assert tidy_marker in tidy_text
assert "src/climate/ClimateApplication.cpp" not in tidy_text
tidy_text = tidy_text.replace(
    tidy_marker,
    tidy_marker + "  src/climate/ClimateIoAdapters.cpp\n  src/climate/ClimateApplication.cpp\n",
    1,
)
tidy.write_text(tidy_text, encoding="utf-8")

status = ROOT / "docs" / "CURRENT_STATUS.md"
status_text = status.read_text(encoding="utf-8")n