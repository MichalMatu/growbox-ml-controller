...
normalize_format() {
  .venv/bin/pre-commit run clang-format --files "${stage27_files[@]}" || true
  .venv/bin/pre-commit run clang-format --files "${stage27_files[@]}"
}
...
case "$MODE" in
  ...
  focused)
    ...
    host_stage27_test
    assert_native_only
    .venv/bin/pre-commit run --files "${stage27_files[@]}"
    host_stage27_test
    source_idf
    build_real
    git diff --check
