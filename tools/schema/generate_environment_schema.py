...
def validate(document: dict[str, Any]) -> None:
    ...
    if version == 2 and len(features) > MAX_FEATURE_COUNT_V2:
        raise ValueError(f"v2 feature count must be <= {MAX_FEATURE_COUNT_V2}")
...
def default_output_for_schema(schema_path: Path) -> Path:
    if schema_path.resolve() == DEFAULT_SCHEMA_V2.resolve():
