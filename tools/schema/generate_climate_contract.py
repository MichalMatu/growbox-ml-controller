...
def validate(document: dict[str, Any]) -> None:
    ...
    for item in [*features, *outputs]:
        ...
        if (
            not all(math.isfinite(v) for v in (lo, hi, default))
            or lo > hi
            or not lo <= default <= hi
        ):
...
def render(document: dict[str, Any]) -> str:
    ...
    feature_enum = "\n".join(
        f"  {cpp_identifier(item['name'])} = {i}U," for i, item in enumerate(features)
    )
    output_enum = "\n".join(
        f"  {cpp_identifier(item['name'])} = {i}U," for i, item in enumerate(outputs)
    )
