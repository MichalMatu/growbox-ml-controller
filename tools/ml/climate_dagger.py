...
class DaggerRows:
    ...
    def __post_init__(self) -> None:
        ...
        if any(
            len(values) != rows
            for values in (self.families, self.humidity_modes, self.safe_fallbacks)
        ):
...
def collect_dagger_rows(
    ...
    *,
    ...
    return DaggerRows(
        dataset=dataset, families=families, humidity_modes=modes, safe_fallbacks=fallbacks
    )
...
def append_train_only(base: Dataset, dagger: Dataset) -> Dataset:
    ...
    if overlap:
        raise ValueError(
            "DAgger scenarios overlap existing dataset: " + ", ".join(sorted(overlap)[:5])
        )
    return Dataset(
