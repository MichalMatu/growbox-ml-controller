...
def main() -> None:
    ...
    for metric_name in ("balanced_mae", "mae", "rmse", "macro_f1"):
        ...
        if delta > 1.0e-6:
            raise AssertionError(f"reproduced {metric_name} differs from report by {delta:.9g}")
    ...
    if prediction_delta > 2.0e-5:
        raise AssertionError(f"portable inference differs from Keras by {prediction_delta:.9g}")
