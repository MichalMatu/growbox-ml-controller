...
def _heartbeat(label: str, stop: threading.Event) -> None:
    ...
    while not stop.wait(60.0):
        print(
            f"STAGE10_HEARTBEAT phase={label} elapsed_s={int(time.monotonic() - started)}",
            flush=True,
        )
...
def _copy_model(
    src_weights: Path, src_metadata: Path, dst_weights: Path, dst_metadata: Path
) -> None:
...
def init() -> None:
    ...
    _save_state(state)
    print(
        f"STAGE10_INIT rows={len(base.features)} dev_tracking={dev.aggregate['ml'].tracking_cost:.6f}",
        flush=True,
    )
