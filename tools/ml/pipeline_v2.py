"""Run the deterministic v2 dataset, training, export, and verification pipeline."""
...
from .contract import V2_CONTRACT_PATH, load_contract
...
def run_pipeline_v2(
    *,
    ...
    mode_artifacts = artifact_root / f"v2-{mode}"
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.check_generated:
        with tempfile.TemporaryDirectory(prefix="growbox-v2-determinism-") as directory:
            root = Path(directory)
            ...
            print("v2 generated model and golden vectors are reproducible")
            return 0

    result = run_pipeline_v2(mode=mode, seed=args.seed, artifact_root=args.artifact_root)
    test_metrics = result.training.metrics["test"]
    print(
        f"v2 {mode} pipeline complete: rows={len(result.dataset.features)}, "
