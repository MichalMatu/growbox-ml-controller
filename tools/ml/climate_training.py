...
def prediction_metrics(
    ...
    *,
    ...
    for index, name in enumerate(CLIMATE_OUTPUT_NAMES):
        ...
        inactive = ~active
        active_mae = (
            float(np.mean(absolute[active])) if np.any(active) else float(np.mean(absolute))
        )
        inactive_mae = (
            float(np.mean(absolute[inactive])) if np.any(inactive) else float(np.mean(absolute))
        )
        balanced = 0.5 * (active_mae + inactive_mae)
        precision, recall, f1 = _binary_metrics(active, predicted[:, index] > activity_threshold)
