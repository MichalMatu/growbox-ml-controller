...
A bounded primary-serial service console is now integrated into the real-input runtime. It keeps output on ESP-IDF primary stdout and consumes command input directly from the ESP-IDF driver for the configured primary console UART, so the qualified CrowPanel uses the same CH340 serial monitor for logs and commands without a separate USB Serial/JTAG channel. It is intentionally separate from semantic actuator binding and does not replace `LockedFakeRoleDriver`. The console provides read-only `help`, `status`, `sensors`, and `rf list` commands plus explicit named manual RF transmit commands for lamp, fan and humidifier and a bounded one-shot RF receive command. Safe numeric aliases `0..3` exist only for read-only menu actions.
