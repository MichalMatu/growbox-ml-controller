...
bool parseUnsigned64(std::string_view token, std::uint64_t& output) noexcept {
  ...
  for (char c : token) {
    ...
    if (value > (std::numeric_limits<std::uint64_t>::max() - d) / 10U)
      return false;
    value = value * 10U + d;
  }
  output = value;
...
bool copyLogFilename(std::string_view token, std::array<char, 16U>& output) noexcept {
  if (token.size() != 11U || token[8] != '.' || lowerAscii(token[9]) != 'j' ||
      lowerAscii(token[10]) != 'l')
    return false;
...
ServiceConsoleCommand invalidCommand() noexcept {
  ServiceConsoleCommand c{};
  c.kind = ServiceConsoleCommandKind::Invalid;
  return c;
}
...
ServiceConsoleCommand parseServiceConsoleCommand(const char* line) noexcept {
  ...
  if (equalsIgnoreCase(tokens[0], "automation")) {
    if (count == 1U || (count == 2U && equalsIgnoreCase(tokens[1], "status"))) {
      command.kind = ServiceConsoleCommandKind::AutomationStatus;
      return command;
    }
    if (count == 2U && equalsIgnoreCase(tokens[1], "on")) {
      command.kind = ServiceConsoleCommandKind::AutomationEnable;
      return command;
    }
    if (count == 2U && equalsIgnoreCase(tokens[1], "off")) {
      command.kind = ServiceConsoleCommandKind::AutomationDisable;
      return command;
    }
    return invalidCommand();
  }
  if (equalsIgnoreCase(tokens[0], "maintenance")) {
    if (count == 1U || (count == 2U && equalsIgnoreCase(tokens[1], "status"))) {
      command.kind = ServiceConsoleCommandKind::MaintenanceStatus;
      return command;
    }
    if (count == 2U && equalsIgnoreCase(tokens[1], "enter")) {
      command.kind = ServiceConsoleCommandKind::MaintenanceEnter;
      return command;
    }
    if (count == 2U && equalsIgnoreCase(tokens[1], "exit")) {
      command.kind = ServiceConsoleCommandKind::MaintenanceExit;
      return command;
    }
    return invalidCommand();
  }
  ...
  if (count == 3U && equalsIgnoreCase(tokens[0], "rtc") &&
    ...
    if (!parseUnsigned64(tokens[2], command.unix_time_s))
      return invalidCommand();
  ...
  if (equalsIgnoreCase(tokens[0], "sdlog")) {
    if (count == 2U && equalsIgnoreCase(tokens[1], "status")) {
      command.kind = ServiceConsoleCommandKind::SdLogStatus;
      return command;
    }
    if (count == 2U && equalsIgnoreCase(tokens[1], "list")) {
      command.kind = ServiceConsoleCommandKind::SdLogList;
      return command;
    }
    if (count == 2U && equalsIgnoreCase(tokens[1], "selftest")) {
      command.kind = ServiceConsoleCommandKind::SdLogSelfTest;
      return command;
    }
    if (count == 5U && equalsIgnoreCase(tokens[1], "read")) {
      if (!copyLogFilename(tokens[2], command.filename) ||
          !parseUnsigned(tokens[3], command.offset) || !parseUnsigned(tokens[4], command.length) ||
          command.length == 0U || command.length > kMaximumSdReadBytes)
        return invalidCommand();
      command.kind = ServiceConsoleCommandKind::SdLogRead;
      return command;
    }
    return invalidCommand();
  }
  if (equalsIgnoreCase(tokens[0], "output")) {
    if (count == 3U && parseDevice(tokens[1], command.device) &&
      ...
      command.kind = ServiceConsoleCommandKind::ManualOutput;
      return command;
    }
    return invalidCommand();
  }
  ...
  if (count == 4U && equalsIgnoreCase(tokens[1], "raw") && parseDevice(tokens[2], command.device) &&
      parseState(tokens[3], command.state)) {
    command.kind = ServiceConsoleCommandKind::MaintenanceRawOutput;
    return command;
  }
  if (equalsIgnoreCase(tokens[1], "rx")) {
    ...
    if (count == 3U) {
      if (!parseUnsigned(tokens[2], command.timeout_ms) ||
          command.timeout_ms < kMinimumRxTimeoutMs || command.timeout_ms > kMaximumRxTimeoutMs)
        return invalidCommand();
      command.kind = ServiceConsoleCommandKind::RfReceive;
      return command;
    }
