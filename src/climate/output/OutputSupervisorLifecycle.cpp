...
OutputSupervisorLifecycle::transition(SupervisorMode next, OutputLifecycleFaultReason fault,
  ...
  return {OutputLifecycleTransitionStatus::Applied,
          before,
          mode_,
          fault_,
          has_policy_event,
...
OutputLifecycleTransitionReport OutputSupervisorLifecycle::noChange() const noexcept {
  return {OutputLifecycleTransitionStatus::NoChange,
          mode_,
          mode_,
          fault_,
          false,
...
OutputSupervisorLifecycle::failClosed(OutputLifecycleFaultReason fault) noexcept {
  ...
  return {OutputLifecycleTransitionStatus::IllegalTransition,
          before,
          mode_,
          fault_,
          true,
...
OutputSupervisorLifecycle::apply(OutputLifecycleCommand command) noexcept {
  if (!valid_) {
    return {OutputLifecycleTransitionStatus::InvalidPolicy,
            mode_,
            mode_,
            fault_,
            false,
  ...
  case OutputLifecycleCommand::ArmingFailed:
    if (mode_ == SupervisorMode::Arming) {
      return transition(SupervisorMode::FaultLocked, OutputLifecycleFaultReason::ArmingFailed, true,
                        OutputLifecycleEvent::Fault);
    }
    break;
